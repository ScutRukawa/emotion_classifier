...
if __name__ == '__main__':
    ...
    X_train, y_train = get_dataset(train_df)
    X_valid, y_valid = get_dataset(dev_df)
    X_test, y_test = get_dataset(test_df)

    state_dim = 32  # 64效果不行
    epochs = 10

    model = Sequential()
    # embedding_dim_train 过大 没有足够的数据
    model.add(Embedding(input_dim=6411, output_dim=embedding_dim_train, input_length=max_sequence_length))
    # model.add(SimpleRNN(units=state_dim, return_sequences=True))  # state_dim 如何调参？
    # model.add(Bidirectional(SimpleRNN(units=state_dim, return_sequences=True)))  # state_dim 如何调参？
    model.add(LSTM(units=state_dim, return_sequences=True))
    # model.add(LSTM(units=state_dim, return_sequences=True))  多层与单层结果基本一致
    model.add(Flatten())
